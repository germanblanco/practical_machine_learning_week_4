...
inTrainVerify <- !(train_valid_sensor_data_df$num_window %in% series_classe[inTrainAll,]$num_window)
...
my_verify_df_and_user_name <- train_valid_sensor_data_df[inVerify,]
my_train_ensemble_df_and_user_name <- train_valid_sensor_data_df[inTrainEnsemble,]
my_train_df_and_user_name <- train_valid_sensor_data_df[inTrain,]
## Remove the num_window column now
my_train_df <- my_train_df_and_user_name[,-length(colnames(my_train_df_and_user_name))]
my_train_ensemble_df <- my_train_ensemble_df_and_user_name[,-length(colnames(my_train_df_and_user_name))]
my_verify_df <- my_verify_df_and_user_name[,-length(colnames(my_verify_df_and_user_name))]
```

### Preprocessing the Data

There are 52 columns of numeric data in the training dataset now. In order to help convergence of algorithms, and in order to be able to compare the relevance of parameters with similar units and in general reduce the complexity of processing, normalization and PCA will be applied. It is assumed that losing interpretability in these complex sensor data is a good tradeoff.

```{r preprocessing_data}
## Preprocessing of data, using all columns except for the last one
preProcessing <- preProcess(my_train_df[,-length(colnames(my_train_df))], method=c("center", "scale", "pca"))
