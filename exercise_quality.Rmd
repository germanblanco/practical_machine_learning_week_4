...
train_data_knn <- cbind(trainPC, predict(dummy, train_csv_df[inTrainModels,]))
...
train_ensemble_data_knn <- cbind(trainEnsemblePC, predict(dummy, train_csv_df[inTrainEnsemble,]))
train_ensemble_data_knn$classe <- train_csv_df[inTrainEnsemble,]$classe
knn_model <- train(classe ~ ., data=train_data_knn, method='knn')
svm_model <- train(classe ~ ., data=train_data_rf, method='svm')
...
train_ensemble_data$prediction_svm <- predict(svm_model, train_ensemble_data_rf)
...
ensemble_model <- train(classe ~ prediction_rf + prediction_knn + prediction_svm, data=train_ensemble_data, method='rf')
train_ensemble_data$prediction_ensemble <- predict(ensemble_model, train_ensemble_data)
predDF <- data.frame(
  train_ensemble_data$prediction_rf,
  train_ensemble_data$prediction_knn,
  train_ensemble_data$prediction_svm,
...
The accuracy in the verification dataset of the best classifier obtained is shown in the following confusion matrix.
